<?php
 ...
 class  Eps_user_epin_datatable extends WP_List_Table {
/**
  ...
  /**
	...
	*/
		public function get_bulk_actions() {
			// pr($tab);
				$actions = array(
					'delete_epin'     => __( 'Delete Epin')	
					
				);
				return apply_filters( 'eps_affiliats_epin_bulk_action', $actions );
			}
	 ...
	 */
		public function process_bulk_actions() {
			...
			if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'bulk-epin-affiliates' ) && ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'epin-affiliate-nonce' ) ) { 
				return;
			}
			$ids = isset( $_GET['pin_id'] ) ? $_GET['pin_id'] : false;
			if ( ! is_array( $ids ) ) {
				$ids = array( $ids );
			}
			...
			foreach ( $ids as $id ) {
				...
				if ( 'delete_epin' === $this->current_action() ) { 
					$response = apply_filters('epsaffliates_epin_delete_my_pin', $id);
					if ( $response ) { 
						echo wp_set_message('E-pin Deleted successfully', 'success');
					} else {
...
function afl_user_epin_view(){
		$epin_affiliates_table = new Eps_user_epin_datatable();
	?>
			<div class="wrap">
			<?php
			/**
			 ...
			 */
				...
				<form id="eps-affiliates-filter" method="get" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
					...
					<input type="hidden" name="page" value="my-e-pin" />
